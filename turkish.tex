...
C, genel amaçlı bir programlama dilidir. C, sistem ve onu çalıştıran birçok programın C dilinde yazılmasından beri geliştirildiği "UNIX" sistemiyle yakından ilişkili olmuştur. Ancak dil herhangi bir işletim sistemi ve makineye bağlı değildir ve derleyiciler (\textbf{compilers}) ve işletim sistemlerini yazmakta kullanışlı olduğu için "sistem programlama dili" olarak adlandırılmış olmasına rağmen aynı ölçüde birçok farklı alanda majör programlar yazmak için de kullanılmıştır.
\par BCPL dilinden kaynaklanan birçok C fikirlerinin çoğu Martin Richards tarafından geliştirildi. BCPL'in C'nin üzerindeki etkisi dolaylı yoldan Ken Thompson tarafından 1970'te DEC PDP-7'deki ilk "UNIX" sistemi için yazılmış olan B dili aracılığıyla devam etti. 
\par BCPL ve B "türsüz" dillerdir. Onların aksine C, veri türünün çeşitliliğini sağlar. Temel türler karakterler, tam sayılar (\textbf{integers}), ve bazı boyuttaki ondalıklı (\textbf{floating-points}) sayılar. Ek olarak; türetilmiş veri türlerinin işaretçiler (\textbf{pointers}), diziler (\textbf{arrays}), yapılar (\textbf{structures}) ve birlikler (\textbf{unions}) ile birlikte yarattığı bir hiyerarşi vardır. İfadeler, operatörler (\textbf{operators}) ve işlenenlerden (\textbf{operands}) oluşur. Her açıklama (\textbf{expression}), bir atama (\textbf{assignment}) veya bir fonksiyon (\textbf{function}) çağrısı da dahil olmak üzere, bir ifade(\textbf{statement}) olabilir. İşaretleyiciler (\textbf{pointers}) makineden bağımsız (\textbf{machine-independent}) adres aritmetiği ihityacını karşılıyor.
\par C, iyi yapılandırılmış programlar için gerekli olan temel kontrol akışı (\textbf{control-flow}) yapılarını sağlar: ifade (\textbf{statement}) gruplaması, karar verme (if-else), mümkün olan durumlar dizisinden birini seçme (switch), tepedeki (while, for) veya alttaki (do) bitiş testi ile döngü oluşturma ve erken döngü (\textbf{loop}) çıkışı (break).
\par Fonksiyonlar (\textbf{functions}), temel türlerin, yapıların (\textbf{structures}), birliklerin (\textbf{unions}) veya işaretleyicilerin (\textbf{pointers}) değerine dönebilir. Her fonksiyon (\textbf{function}) "yinelemeli" olarak adlandırılabilir. Yerel değişkenler (\textbf{variables}) tipik olarak otomatikler ya da her çağrı ile yeniden yaratıldılar. Fonksiyon (\textbf{function}) tanımları iç içe olmayabilir fakat değişkenler (\textbf{variables}) blok yapı (\textbf{block-structured}) tarzında tanımlanabilir. C programının fonksiyonları (\textbf{functions}) ayrı olarak derlenmiş ayrı kaynak dosyaları içinde bulunabilir. Değişkenler; (\textbf{variables}) bir fonskiyon (\textbf{function}) için dahili, harici fakat sadece tek bir kaynak dosyası içinde bilinen ya da tüm program için gözle görülür olabilirler. 
\par Bir önişlem adımı program metninde; makro yer değiştirme, diğer kaynak dosyalarının dahil edilmesi ve şartlı derleme gerçekleştirir. 
\par C, nispeten "düşük level" bir dil. Bu nitelendirme aşağılayıcı değil, sadece birçok bilgisayarın yaptığı aynı tür nesneler ile uğraşır. Yani karakterler, sayılar ve adresler. Bunlar gerçek makineler tarafından uygulanan aritmetik ve mantıksal operatörler (\textbf{operators}) tarafından birleştirilebilir ve hareket ettirilebilirler. C, karakter dizileri, kümeler, listeler ve dizilişler gibi karışık objelerle uğraşmak için işlemler sağlamaz. Yapıların (\textbf{structures}) bir birim olarak kopyalanabilmesine rağmen tüm dizi (\textbf{string}) ve dizilişleri (\textbf{arrays}) idare edebilecek işlemler yoktur. Dil, statik tanımlama ve fonksiyonların (\textbf{functions}) yerel değişkenleri (\textbf{variables}) tarafından sağlanan yığın denetimi dışında hiçbir depolama tahsis olanağı tanımlaması olmayan bir dildir. Yığın veya değersiz veri koleksiyonu yoktur. Son olarak C'nin kendisi girdi (\textbf{input})/çıktı (\textbf{output}) olanağı sağlamaz, okuma veya yazma ifadeleri (\textbf{statements}) ve yerleşik dosya erişim metotları yoktur. Bütün bu yüksek seviye mekanizmaların açıkça adlandırılmış fonksiyonlar (\textbf{functions}) tarafından sağlanması gerekir. Birçok C uygulaması, bu fonksiyonların (\textbf{functions}) uygun bir şekilde standart koleksiyonunu içermiştir. 
\par Benzer şekilde C sadece basit, tek dişli kontrol akışı (\textbf{control-flow}) sunar: testler, döngüler (\textbf{loops}), gruplama ve alt programlar. Fakat multi programlama, paralel işlemler, senkronizasyon ya da eşyordamlar değil. Bu özelliklerin bazılarının yokluğu ölü gereksinim gibi gözükse de ("İki karakter dizisini karşılaştırmam için bir fonksiyon (\textbf{function}) çağırmam gerektiğini mi söylemek istedin?") dili mütevazi boyutlarda tutmann gerçek faydaları vardır. C nispeten küçük bir dil olduğundan beri, küçük bir alanda tanımlanabiliyor ve hızlıca öğrenilebiliyor. Bir programcı mantıklı olarak tüm dili bilmeyi, anlamayı ve aslında düzenli olarak kullanmayı bekleyebilir.
\par Yıllarca C'nin tanımı, C Programlama Dili'nin ilk tirajındaki referans el kitabı oldu. 1983'te, Amerikan Standartları Enstitüsü (\textbf{ANSI}) C'nin modern ve kapsamlı bir tanımını sağlamak için bir komite kurdu. Ortaya çıkan tanım, "ANSI standardı" ya da "ANSI C" 1988'in sonlarına doğru tamamlandı. Standardın birçok özelliği çoktan modern derleyiciler (\textbf{compilers}) tarafından desteklenmiştir.
\par Birçok programcı için en önemli değişim; fonksiyonları (\textbf{functions}) bildirmek ve tanımlamak için olan yeni sözdizimi. Bir fonksiyon (\textbf{function}) tanımlaması şimdi fonksiyonun (\textbf{function}) argümanlarının (\textbf{arguments}) açıklamasını içerebilir; tanımlama sözdizimi eşleşmek üzere değişir. Bu ekstra bilgilendirme derleyicileri (\textbf{compilers}) eşleşmeyen argümanlar (\textbf{arguments}) tarafından ortaya çıkan hataları belirlemek için daha kolay yapar. Bizim deneyimimizde, dile yapılan son derece kullanışlı bir ekleme.
\par Daha küçük çaplı dil değişimleri var. Geniş ölçüde mevcut olan yapı atamaları (\textbf{structure assignment}) ve numaralandırmalar şimdi resmi olarak dilin bir parçası. Ondalıklı sayı (\text{floating-point numbers}) hesaplamaları şimdi tek duyarlıklı (\textbf{single precision}) formatta yapılabilir. Aritmetiğin özellikleri, özellikle imzasız türler netleştirildi. Önişlemci daha ayrıntılı. Bu değişimlerin çoğu, birçok programcı üzerinde sadece minör etkiye sahip olacaktır.
\par Standardın ikinci en önemli katkısı, C'ye eşlik etmek üzere bir kütüphanenin tanımıdır. Bu; işletim sistemine erişmek için fonksiyonlar (\textbf{functions}) (örneğin dosyaları okumak ve yazmak için), biçimlendirilmiş girdi (\textbf{input}) ve çıktı (\textbf{output}), hafıza tahsisi, dizi manipülasyonu vb. şeyler belirtir. Standart başlıkların koleksiyonu, fonksiyonların (\textbf{functions}) ve veri türlerinin tanımlamalarına tek tip erişim sağlar. Bu kütüphaneyi sunucu bilgisayar ile etkileşim için kullanan programlar, uyumlu davranışların güvencesi altına alınmıştır. Kütüphanenin büyük bir kısmı "standart I/O kütüphanesinde" dikkatle modellenmiştir. Bu kütüphane ilk tirajda açıklanıyor ve diğer sistemlerde de geniş ölçüde kullanılmıştır. Tekrar söylüyoruz, birçok programcı çok fazla değişim görmeyecek.
\par C tarafından sağlanan veri türleri ve kontrol yapılarının (\textbf{structures}) birçok bilgisayar tarafından doğrudan destekleniyor olması yüzünden kendine yeten programları uygulamak için gerekli olan çalışma kütüphanesi oldukça küçük. Standart kütüphane fonksiyonları (\textbf{functions}) sadece açıkça çağrılıyor. Bu yüzden gerekli olmadıkları durumlarda kaçınılabiliyorlar. Çoğu, C dilinde yazılabilir ve gizledikleri işletim sistemi detayları hariç kendileri taşınabilirler.
\par C, birçok bilgisayarın kabiliyetleri ile uyuşsa da belirli herhangi bir makine mimarisinden bağımsızdır. Biraz özenle, taşınılabilir programlar yazmak için kolaydır. Yani donanımın değişkeni (\textbf{variable}) üzerinde bir değişim olmadan çalışabilecek programlar. Standart taşınılabilirlik sorunlarını açık hale getirmekte ve programın üzerinde çalıştırıldığı makineyi tanımlayan sabitler kümesini reçete etmekte.
\par C, kuvvetli yazılmış bir dil değil fakat tıpkı geliştiği gibi tür kontrolü de güçlendirildi. C'nin orijinal tanımı, işaretliyiciler (\text{pointers}) ve tam sayıların (\textbf{integers}) değişimini hoş karşılamadı fakat izin verildi. Bu ortadan kaldırılalı uzun zaman oldu ve şimdi standart, iyi derleyiciler (\textbf{compilers}) tarafından çoktan zorunlu olan uygun tanımlamalar ve açık dönüşümler gerektiriyor. Yeni fonksiyon (\textbf{function}) tanımlamaları bu yönetimdeki bir başka adım. Derleyiciler (\textbf{compilers}) birçok tür hatasını uyarır ve uyumsuz veri türlerinin otomatik dönüşümleri yoktur. Yine de C, programcıların ne yaptığını bildiği basit felsefeyi korur, sadece onların amaçlarını açıkça bildirmelerini gerektirir.
\par C'nin de başka her dilin olduğu gibi kendi kusurları var. Operatörlerin (\textbf{operators}) bazıları yanlış önceliklere sahip. Söz diziminin bazı kısımları daha iyi olabilir. Yine de C, programlama uygulamalarının geniş çeşitliliği için son derece etkili ve anlamlı bir dil olarak kanıtlanmıştır.
\par Bu kitap şu şekilde organize edilmiştir: Bölüm 1, C'nin merkezi kısmında öğreticidir. Biz; yeni bir dil öğrenmenin o dilde programlar yazmak olduğuna inandığımızdan beri amaç, mümkün olduğunca çabuk başlayan okuyucular edinmek. Öğretici kısım; programlamanın temel elementlerinin çalışma bilgisini üstlenir: Ne bilgisayarların, ne derlemenin ne de "n=n+1" gibi bir ifadenin anlamının açıklaması var. Biz mümkün olan yerlerde kullanışlı programlama tekniklerini göstermeyi denememize rağmen bu kitap veri yapıları (\textbf{structures}) ve algoritmalar üzerinde referans çalışması olarak tasarlanmamıştır. Bir seçim yapmak zorunda kaldığımızda, dil üzerinde yoğunlaştık.
Üzerinde durulan nokta hala yalıtılmış kısımlardan ziyade tamamlanmış programların örnekleri olmasına rağmen Bölüm 2, Bölüm 6 ile beraber C'nin çeşitli yönlerini Bölüm 1'de olduğundan daha detaylı ve daha biçimsel olarak tartışıyor. Bölüm 2, temel veri türleri, operatörler (\textbf{operators}) ve ifadeler (\textbf{statements}) ile ilgilenir. Bölüm 3, kontrol akışını (\textbf{control-flow}) ele alır: if-else, switch, while, for vb. Bölüm 4, fonksiyonlar (\textbf{functions}) ve program yapılarını (\textbf{structures}) -harici değişkenler (\textbf{variables}), kapsam kuralları, çoklu kaynak dosyaları ve bunun gibi- kapsar ve ayrıca önişkemciye değinir. Bölüm 5 işaretçileri (textbf{pointers}) ve adres aritmetiğini tartışır. Bölüm 6 yapıları (\textbf{structures}) ve birlikleri (\text{unions}) kapsar.
\par Bölüm 7, işletim sistemine uygun arayüz sağlayan standart kütüphaneyi tanımlar. Bu kütüphane "ANSI" standardı tarafından tanımlanmıştır ve C'yi destekleyen tüm makinelerde destekleniyor olması gerekir. Bu yüzden onu girdi (\textbf{input}), çıktı (\textbf{output}) ve diğer işletim sistemi erişimi için kullanan programlar, bir sistemden diğerine değiştirilmeden taşınabilir.
\par Bölüm 8, C programları ve UNIX işletim sistemi, girdi (\textbf{input}) ve çıktıya (\textbf{output}) odaklanma, dosya sistemi ve depolama tahsisi arasındaki arayüzü tanımlar. Bu bölümün bir kısmı UNIX sistemi için spesifik olsa da diğer sistemleri kullanan programcılar da burada standart kütüphanenin bir versiyonunun nasıl uygulandığını kavrama ve taşınılabilirlik önerileri de dahil olmak üzere kullanışlı materyaller bulabilirler. Ek Bölüm A, bir dil referans el kitabı içerir. Sözdiziminin ve C'nin anlambiliminin resmi ifadeleri (\textbf{statements}) ANSI standardıdın kendisidir. Ancak bu belge başta derleyici (\textbf{compiler}) yazıcılara yöneliktir. Buradaki referans el kitabı dilin tanımlamasını daha kısa ve aynı legal stil olmadan iletir. Ek Bölüm B, uygulayıcılardan çok kullanıcılar için standart kütüphanein bir tanımı. Ek Bölüm C, orijinal dildeki değişimlerin kısa bir özeti. Ancak şüphe durumunda standart ve kendi derleyicisi (\textbf{compiler}), dilin son yetkilileri olarak kaldılar.
