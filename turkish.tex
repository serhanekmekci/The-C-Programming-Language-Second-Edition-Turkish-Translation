...
\textbf{Egzersiz 1-3.} Isı birimi dönüşüm programını tablonun başındaki yorum satırlarındaki başlığı bastıracak şekilde düzenleyin. \\
...
\textbf{Egzersiz 1-8.} Boşlukları, tableri, ve yenı satırları sayan bir program yazınız. \newline

\textbf{Egzersiz 1-9.} Girdisini (\textbf{input}) çıktısına (\textbf{output}) kopyalayan bir program yazınız, her karakter öbeğindeki (\textbf{string}) birden fazla olan boşlukları tek boşlukla değiştirecek. \newline

\textbf{Egzersiz 1-10.} Girdisini (\textbf{input}) çıktısına (\textbf{output}) kopyalayan bir program yazınız, her tabi \textbackslash t ile, her geri tuşunu \textbackslash b ile ve her ters eğik çizgiyi \textbackslash \textbackslash ile değiştirecek. Bu tableri, ters eğik çizgiyi ve geri tuşu karakterlerini daha açık bir yolla gösterecek.
...
\begin{lstlisting}
		if (c == ' ' || c == '\n' || c == '\t')
\end{lstlisting}
şunu söylüyor "c bir boşluk veya c bir yeni satır veya c bir tab ise". (Kaçış dizesi (\textbf{escape sequence}) olan \textbackslash t'nin tab karakterinin görünür temsili olduğunu belirtelim) VE'nin (\textbf{AND}) yerine geçen bir operatör var \&\&; ve önceliği ||'den büyük.
\end{document}
