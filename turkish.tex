...
Bilgi işlem dünyası, 1978 yılında "C Programlama Dili"nin yayınlanmasından itibaren köklü bir devrime uğramıştır. Büyük bilgisayarlar artık daha da büyükler ve kişisel bilgisayarlar artık on yıl öncesinin ana bilgisayarlarına rakip olabilecek kapasitedeler. Bu zaman içerisinde, mütevazi bir şekilde olsa da C de değişime uğramış ve kendi orijini olan UNIX işletim sistemi olmaktan çok uzaklara yayıldı.
\par C'nin gittikçe artan popülaritesi, yıllar içerisinde dilde gerçekleşen değişimler ve gruplar tarafından derleyicilerin oluşturulması onun dizaynına bulaştırılmadı; dilin daha kusursuz ve kitabın ilk tirajına göre daha çağdaş bir tanımı için kombine edildi. 1983'te Amerikan Ulusal Standartları Enstitüsü amacı, C dilinin ruhunu kaybetmeden kesin ve makineye dayalı olmayan bir tanımını üretmek olan bir komite kurmuştur. Sonuçlar, C için "ANSI" standardıdır.
\par Standart, ilk tirajda bahsi geçmiş fakat tam anlamıyla tanımlanmamış yapıları şekillendiriyor. Özellikle yapı ataması ve numaralandırma. Bu; tanımın, kullanımla birlikte çapraz kontrolüne izin veren bir fonksiyon ifadesinin yeni bir formunu sağlar. Girdi ve çıktıları sergilemek için olan fonksiyonların geniş bir dizisi, hafıza yönetimi, dize manipülasyonu ve benzer görevlerle birlikte standart bir kütüphane belirtir. Orijinal tanımlamada ayrıntılı olarak açıklanmamış özelliklerin davranışlarını kesin yapar ve aynı zamanda dilin makineye dayalı kalan yönlerini açık bir şekilde belirtir.
\par C Programlama Dili'nin ikinci tirajı, C'yi ANSI standartlarında olduğu gibi tanımlar. Biz, dilin geliştiği yerleri not etmiş olmamıza rağmen sadece yeni formda yazmayı tercih ettik. Büyük bir bölüm için bu, çok önemli bir fark yaratmaz. En gözle görülür değişim, fonksiyon bildirim ve tanımının yeni formudur. Modern derleyiciler birçok standart özelliğini çoktan desteklemektedirler.
\par Kitabı ilk tirajın kısalığında tutmaya çalıştık. C, büyük bir dil değil ve büyük bir kitap tarafından sunulmuyor. Biz, işaretçiler gibi kritik özelliklerin sunumunu geliştirdik. Bunlar C programlaması için merkezidir. Orijinal örnekler işledik ve birkaç bölüme yeni örnekler ekledik. Örneğin; tanımlamaları kelimelere dönüştüren ve bunun tersini yapan programlar tarafından komplike tanımlamalar oluşturuldu. Önceden olduğu gibi bütün örnekler doğrudan, makine tarafından okunulabilir formda olan metinden test edilmiştir.
\par Ek Bölüm A; referans el kitabı, standart değildir. Fakat bizim girişimimiz standardın gerekliliklerini daha küçük bir alanda iletmek. Bu programcılar tarafından kolay anlaşılmak üzere yapıldı ancak derleyici yazarlar için bir tanım olarak değil. - Bu rol tamamen standardın kendisine ait. Ek Bölüm B, standart kütüphanenin imkanlarının özetidir. Bu uygulayıcılar değil programcılar tarafından referans için yapıldı. Ek Bölüm C, orijinal versiyondaki değişimlerin kısa özetidir.
\par İlk tirajın önsözünde de söylediğimiz gibi C "sizlerin gelişen tecrübesiyle hala iyi kalmayı başarıyor." On yılın getirdiği tecrübe ile birlikte biz hala bu şekilde hissediyoruz. Umuyoruz ki bu kitap, C'yi öğrenmenizde ve iyi bir şekilde kullanmanızda yardımcı olur.
\par Bu ikinci tirajı ortaya çıkarmamızda yardımcı olan arkadaşlarımıza derinden borçlu hissediyoruz.  Jon Bentley, Doug Gwyn, Doug Mcllroy, Peter Nelson ve Rob Pike bize neredeyse taslağın ger sayfasında sezgilerinden yola çıkarak yorumlarda bulundular. Dikkatli okumaları için  Al Aho, Dennis Allison, Joe Campbell, G. R. Emlin, Karen Fortgang, Allen Holub, Andrew Hume, Dave Kristol, John Linderman, Dave Prosser, Gene Spafford, ve Chris Van Wyk'e minnettarız. Ayrıca  BHl Cheswick, Mark Kernighan, Andy Koenig, Robin Lake, Tom London, Jim Reeds, Clovis Tondo, ve Peter Weinberger'den yardımcı tavsiyeler aldık. Dave Prosser ANSI standartları hakkında birçok detaylı sorumuzu cevapladı. Programlarımızın yerel testleri için yaygın olarak  Bjarne Stroustrup'ın C++ çeviricisini kullandık ve Dave Kristol bize final testi için bir ANSI C derleyicisi sağladı. Rich Drechsler dizgilerde bize son derece yardımcı oldu.
